      ...
      >
        {propTabs.map((tab, idx) => (
          <button
            ...
            className={cn("relative px-2 py-2 rounded-full h-auto", tabClassName)}
            style={{
          ...
          >
            {active.value === tab.value && (
              <motion.div
                ...
                className={cn(
                  "absolute inset-0 bg-gray-950 hover:bg-gray-950/50 rounded-full ",
                  activeTabClassName
                )}
              />
            )}

            <span className="relative block text-white font-caveat text-md sm:text-xl">
              {tab.title}
            </span>
          </button>
        ))}
